...
def create_database():
    con=driver.connect(host='localhost',user='root', passwd='MYSQL', charset='utf8')
    if con.is_connected():
...
def create_table():
    con=driver.connect(host='localhost',user='root',passwd='MYSQL',charset='utf8',database='employee')
    if con.is_connected():
        print("Successfully Connected")
    cur=con.cursor()
    ...
    print("+_____________|_____________|________|+")
    print("+Column Name  |DataType(Size)|NULL       |")
    print("+_____________|_____________|________|+")
    for i in cur:
        print('|{0:28}     {1:15}     {2:10}|'.format(i[0],i[1],i[2])) 
    print("+_____________|_____________|________|+")
    con.close()
